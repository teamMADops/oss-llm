...
export async function getFailedStepsAndPrompts(
  ...
  const zipRes = await octokit.request(
    ...
    { owner, repo, run_id, request: { responseType: 'arraybuffer' as any } }
  );
  ...
  for (const f of txtFiles) {
    ...
    const snippet = extractRelevantLog(raw, logMode);
  ...
  if (prompts.length === 0 && txtFiles.length > 0) {
    ...
    const raw = await f.async('string');
    const snippet = extractRelevantLog(raw, logMode);
    prompts.push(formatPrompt({ filename: f.name, logSnippet: snippet }));
  }

...
export function activate(context: vscode.ExtensionContext) {
  ...
  const showRepository = async () => {
    ...
    vscode.window.showInformationMessage(
      `Current repository: ${cur ? cur.owner + "/" + cur.repo : "(none)"}`
    );
  };
  ...
  const loginGithub = async () => {
    ...
    if (ok) {
      ...
      vscode.window.showInformationMessage(
        before ? `You are already logged in as ${who}` : `Successfully logged in as ${who}`
      );
    } else {
  ...
  const analyzeGitHubActions = async (repoArg?: RepoInfo) => {
    ...
    if (!repo) {
      vscode.window.showWarningMessage(
        "No repository found. Please register one first."
      );
      return;
    }
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
        try {
          ...
          printToOutput(`Run #${run_id} failed steps`, failedSteps);
          printToOutput(`Run #${run_id} → LLM prompts`, prompts);
          ...
          printToOutput("LLM analysis result", [JSON.stringify(analysis, null, 2)]);
          ...
          if (panels["dashboard"]) {
            ...
            vscode.window.showInformationMessage(
              "LLM analysis result has been displayed on the dashboard."
            );
          } else {
            ...
            await vscode.window.withProgress(
              ...
              async (progress) => {
                try {
                  ...
                  printToOutput(`Run #${runId}  Failed steps`, failedSteps);
                  printToOutput(`Run #${runId} → LLM prompts`, prompts);
        ...
        case "analyzeSecondPass":
          try {
            ...
            const input: SecondPassInput = {
              ...
              // 출력/전달
              printToOutput("LLM 2nd Pass Analysis Result", [JSON.stringify(result, null, 2)]);
...
function ensureWorkflowPathFromWorkflow(wf: any) {
  if (!wf?.path) throw new Error("Unable to find the workflow path.");

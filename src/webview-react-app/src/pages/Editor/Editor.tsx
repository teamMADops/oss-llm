...
const Editor: React.FC<EditorProps> = ({ actionId }) => {
  ...
  const [workflowTriggers, setWorkflowTriggers] = useState({
    ...
    pull_request: { types: ['opened', 'synchronize'], branches: ['main'], paths: [], enabled: false },
  });
  ...
  const [mainPanelDropdowns, setMainPanelDropdowns] = useState<{ [key: string]: boolean }>({});
  ...
  const toggleMainPanelDropdown = (dropdownKey: string) => {
    setMainPanelDropdowns(prev => ({ ...prev, [dropdownKey]: !prev[dropdownKey] }));
  };
  ...
  return (
    <div className="editor-container">
      ...
      {/* 우측 YAML 미리보기 */}
      <YamlViewer yamlContent={workflowContent} />
    </div>
  );
};

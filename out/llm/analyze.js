...
const suspects_1 = require("./suspects");
const systemPrompts_1 = require("./prompts/systemPrompts");
const prompts_1 = require("./prompts/prompts");
const logPreprocess_1 = require("./logPreprocess");
const llmCache_1 = require("./cache/llmCache");
...
async function getOpenAIKey(context) {
    // 사용자가 등록한 키 확인
...
async function performSingleAnalysis(client, prompt) {
    const safeLog = (0, logPreprocess_1.preprocessLogForLLM)(prompt, {
        maxTokens: 16000,
        safetyMargin: 1500,
        tailCount: 700,
    });
    const userPrompt = (0, prompts_1.buildFirstPassPrompt)(safeLog);
    // 캐시 키 구성
    const cacheKey = {
        namespace: "first-pass",
        model: "gpt-3.5-turbo",
        systemPromptVersion: "fp-2025-10-12", // 날짜나 버전 문자열은 SYSTEM_PROMPTS 버전과 맞춰줘
        prompt: userPrompt,
    };
    const parsed = await llmCache_1.llmCache.getOrCompute(cacheKey, async () => {
        const chat = await client.chat.completions.create({
            model: "gpt-3.5-turbo",
            temperature: 0,
            messages: [
                systemPrompts_1.SYSTEM_PROMPTS.FIRST_PASS,
    ...
    if (!parsed.suspectedPaths || parsed.suspectedPaths.length === 0) {
        ...
        const suspects = (0, suspects_1.extractSuspects)(prompt, { max: 6, excerptPadding: 30 });
        if (suspects.length) {
...
async function analyzePrompts(context, prompts) {

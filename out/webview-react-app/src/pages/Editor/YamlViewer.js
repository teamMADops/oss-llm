...
require("./YamlViewer.css");
const YamlViewer = ({ yamlContent, highlightedLines = [] }) => {
    // YAML 구문 하이라이팅 함수
    ...
    const renderYamlWithLineNumbers = () => {
        ...
        return paddedLines.map((line, index) => {
            ...
            return (<div key={index} className={`yaml-line ${isHighlighted ? 'highlighted' : ''} ${isEmptyLine ? 'empty-line' : ''}`}>
        ...
        <div className="yaml-content">
          <div className="yaml-code-container">
            {paddedLines.map((line, index) => {
                const lineNumber = index + 1;
                const isEmptyLine = line === '';
                return (<div key={index} className={`yaml-line ${isEmptyLine ? 'empty-line' : ''}`}>
                  <span className="line-number">{lineNumber}</span>
                  <span className="line-content" dangerouslySetInnerHTML={{
                        __html: isEmptyLine ? '' : highlightYaml(line)
                    }}/>

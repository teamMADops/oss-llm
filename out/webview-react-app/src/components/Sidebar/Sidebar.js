...
const Sidebar = ({ actions, selectedActionId, activePage, sidebarCollapsed, dropdownActive, actionHighlighted, onSelectAction, onSelectPage, onSidebarToggle }) => {
    return (<>
      <div className={`editor-sidebar ${sidebarCollapsed ? 'collapsed' : ''}`}>
        ...
        <div className="actions-list">
          {actions.map((action) => (<div key={action.id}>
              <div className={`action-item ${selectedActionId === action.id && actionHighlighted ? 'selected' : ''}`} onClick={() => onSelectAction(action.id)}>
              ...
              {selectedActionId === action.id && dropdownActive && (<div className="action-dropdown">
                    <div className={`dropdown-item ${activePage === 'editor' ? 'selected' : ''}`} onClick={() => onSelectPage('editor')}>
                      {/* 아이콘은 나중에 SVG 파일로 교체하는 것이 좋습니다. */}
                      <span className="dropdown-icon">🔧</span>
                      <span>Editor</span>
                    </div>
                    <div className={`dropdown-item ${activePage === 'history' ? 'selected' : ''}`} onClick={() => onSelectPage('history')}>
                      <span className="dropdown-icon">📋</span>
      ...
      {sidebarCollapsed && (<div className="sidebar-collapsed-arrow" onClick={onSidebarToggle}>
